(function(){
	...
	b.prototype = {
		init:function(){
			...
			$('#box').css({'max-height':t.w_height-t.c_height,'max-width':t.w_width-t.c_width,'position':'fixed'});
			$(window).resize(function(){
				...
				$('#box').css({'max-height':t.w_height-t.c_height,'max-width':t.w_width-t.c_width});
				t.height = t.t.outerHeight();
				t.width = t.t.outerWidth();
				if(t.width + t.left >= t.w_width){
					t.left = t.w_width - t.width;
					if(parseInt(t.t.css('left'))<0){
						$('#box').css('left',-t.width+t.hide);
					}else if(parseInt(t.t.css('left'))>t.w_width-t.width){
						$('#box').css('left',t.w_width-t.hide);
					}else{
						$('#box').css('left',t.left);
					}
				}
		...
		move:function(){
			...
			t.cs.on('mousedown',function(){
				t.top = 10;
				t.left =1080;
				t.t.animate({top:10,left:1080},300,function(){
					t.top_animate();
				});
				return false;
			});
			t.c.on('mousedown',function(e){
				...
				$('#box').on('mousemove',function(e){
					...
					$('#box').css({'top':t.top2,'left':t.left2});
					return false;
				});
